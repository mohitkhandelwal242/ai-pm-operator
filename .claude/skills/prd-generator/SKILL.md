...
argument-hint: "[feature name/idea] [--draft] [--no-confluence] [--tickets]"
...
- `--tickets`: After the PRD is approved, create the Jira Epic + a Story per user story (closes spec → backlog).
...
## Phase 4b — Create Jira Epic + Stories (only if `--tickets`)

Turn the approved spec into backlog so the PM doesn't re-key it by hand. **Draft-before-publish:** show the user the exact Epic + Story list you intend to create and get an explicit "yes" first.

1. Create the Epic:
   ```bash
   python3 tools/jira-api.py create --type Epic --summary "[Feature Name]" \
     --description-file docs/specs/prd-[feature-slug].md \
     --labels prd,ai-pm-operator
   ```
   Capture the returned Epic key (e.g. `${PROJECT_KEY}-456`).
2. Create one Story per row in the **Functional Requirements & User Stories** table, parented to the Epic:
   ```bash
   python3 tools/jira-api.py create --type Story --parent ${PROJECT_KEY}-456 \
     --summary "US-01: <story summary>" \
     --description "As a <persona> I want <action> so that <value>. AC: <ac ref from PRD>" \
     --labels prd,ai-pm-operator
   ```
3. Write the real Epic key back into the PRD's **Document Control** (replace `[EPIC_ID_PLACEHOLDER]`) and re-sync Confluence if it was published.
4. Every created issue must carry the `ai-pm-operator` label and a description line *"Generated by AI-PM Operator — prd-generator."*
5. Print the created Epic + Story keys/links.

---

## Phase 5 — Handoff
Print the local markdown file path, Confluence link (if published), and any Jira Epic/Story keys (if `--tickets`) to the terminal for immediate access by the user.
